    ...
    public class FastZip
    {
                       ...
                       bool isStreamOwner)
        {
            ...
            using (zipFile_ = new ZipFile(inputStream))
            {
                ...
                {
                    MyZipEntry entry = (MyZipEntry)enumerator.Current;
                    if (entry.IsFile)
                    {
        ...
        #region 单个文件单个文件解压 add : xhl

        public uint GetNumInZipFile()
        {
            uint res = 0;
            if (zipFile_ != null)
            {
                res = (uint)zipFile_.Count;
            }
            return res;
        }
        ...
        public bool ExtractZipSelf()
        {
            ...
            try
            {
                ...
                if (continueRunning_ && enumerator_.MoveNext())
                {
                    MyZipEntry entry = (MyZipEntry)enumerator_.Current;
                    if (entry.IsFile)
                    {
                        if (directoryFilter_.IsMatch(Path.GetDirectoryName(entry.Name)) && fileFilter_.IsMatch(entry.Name))
                        {
                            ...
                            curExtraZipFileNum += 1;
                            if (GetNumInZipFile() == curExtraZipFileNum)
                                res = true;
                        }
                    }
                    else if (entry.IsDirectory)
        ...
        void ProcessDirectory(object sender, DirectoryEventArgs e)
        {
            if (!e.HasMatchingFiles && CreateEmptyDirectories)
            {
                ...
                if (e.ContinueRunning)
                {
                    ...
                    {
                        MyZipEntry entry = entryFactory_.MakeDirectoryEntry(e.Name);
        ...
        void ProcessFile(object sender, ScanEventArgs e)
        {
            ...
            if (e.ContinueRunning)
            {
                try
                {
                    ...
                    {
                        MyZipEntry entry = entryFactory_.MakeFileEntry(e.Name);
                        outputStream_.PutNextEntry(entry);
        ...
        void ExtractFileEntry(MyZipEntry entry, string targetName)
        {
            bool proceed = true;
        ...
        void ExtractEntry(MyZipEntry entry)
        {
        ...
        #region Instance Fields
        uint curExtraZipFileNum = 0;
        System.Collections.IEnumerator enumerator_;
