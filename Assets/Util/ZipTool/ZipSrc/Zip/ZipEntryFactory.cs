...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class ZipEntryFactory : IEntryFactory
	{
		#region Enumerations
		/// <summary>
		/// Defines the possible values to be used for the <see cref="MyZipEntry.DateTime"/>.
		/// </summary>
		...
		/// </summary>
		/// <param name="timeSetting">The <see cref="TimeSetting">time setting</see> to use when creating <see cref="MyZipEntry">Zip entries</see>.</param>
		...
		/// </summary>
		/// <param name="time">The time to set all <see cref="MyZipEntry.DateTime"/> values to.</param>
		...
		#region Properties
		/// <summary>
		/// Get / set the <see cref="INameTransform"/> to be used when creating new <see cref="MyZipEntry"/> values.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Make a new <see cref="MyZipEntry"/> for a file.
		/// </summary>
		/// <param name="fileName">The name of the file to create a new entry for.</param>
		/// <returns>Returns a new <see cref="MyZipEntry"/> based on the <paramref name="fileName"/>.</returns>
		public MyZipEntry MakeFileEntry(string fileName)
		{
		...
		/// <returns>Returns a new <see cref="MyZipEntry"/> based on the <paramref name="fileName"/>.</returns>
		public MyZipEntry MakeFileEntry(string fileName, bool useFileSystem) {
			return MakeFileEntry(fileName, null, useFileSystem);
		}

		/// <summary>
		/// Make a new <see cref="MyZipEntry"/> from a name.
		/// </summary>
		/// <param name="fileName">The name of the file to create a new entry for.</param>
		/// <param name="entryName">An alternative name to be used for the new entry. Null if not applicable.</param>
		/// <param name="useFileSystem">If true entry detail is retrieved from the file system if the file exists.</param>
		/// <returns>Returns a new <see cref="MyZipEntry"/> based on the <paramref name="fileName"/>.</returns>
		public MyZipEntry MakeFileEntry(string fileName, string entryName, bool useFileSystem)
		{
			MyZipEntry result = new MyZipEntry(nameTransform_.TransformFile(entryName != null && entryName.Length > 0 ? entryName : fileName));
		...
		/// <summary>
		/// Make a new <see cref="MyZipEntry"></see> for a directory.
		/// </summary>
		/// <param name="directoryName">The raw untransformed name for the new directory</param>
		/// <returns>Returns a new <see cref="MyZipEntry"></see> representing a directory.</returns>
		public MyZipEntry MakeDirectoryEntry(string directoryName)
		{
		...
		/// <param name="useFileSystem">If true entry detail is retrieved from the file system if the file exists.</param>
		/// <returns>Returns a new <see cref="MyZipEntry"></see> representing a directory.</returns>
		public MyZipEntry MakeDirectoryEntry(string directoryName, bool useFileSystem)
		{
			
			MyZipEntry result = new MyZipEntry(nameTransform_.TransformDirectory(directoryName));
            result.IsUnicodeText = isUnicodeText_;
            result.Size = 0;
