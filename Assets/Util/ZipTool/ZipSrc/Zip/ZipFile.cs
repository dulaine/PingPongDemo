...
namespace ICSharpCode.SharpZipLib.Zip 
{
	...
	public class TestStatus
	{
		...
		internal void SetEntry(MyZipEntry entry)
		{
			entry_ = entry;
			entryValid_ = true;
			bytesTested_ = 0;
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public MyZipEntry this[int index] 
		{
			get {
				return (MyZipEntry) entries_[index].Clone();	
			}
		}
		...
		/// The Zip file has been closed.
		/// </exception>
		public MyZipEntry GetEntry(string name)
		{
			if (isDisposed_) {
				throw new ObjectDisposedException("ZipFile");
			}			
			...
			return (index >= 0) ? (MyZipEntry) entries_[index].Clone() : null;
		}

		/// <summary>
		/// Gets an input stream for reading the given zip entry data in an uncompressed form.
		/// Normally the <see cref="MyZipEntry"/> should be an entry returned by GetEntry().
		/// </summary>
		/// <param name="entry">The <see cref="MyZipEntry"/> to obtain a data <see cref="Stream"/> for</param>
		/// <returns>An input <see cref="Stream"/> containing data for this <see cref="MyZipEntry"/></returns>
		/// <exception cref="ObjectDisposedException">
		...
		/// </exception>
		public Stream GetInputStream(MyZipEntry entry)
		{
			if ( entry == null ) {
		...
		long TestLocalHeader(MyZipEntry entry, HeaderTest tests)
		{
			lock(baseStream_) 
			{
		...
		/// <summary>
		/// Get/set the <see cref="IEntryFactory"/> used to generate <see cref="MyZipEntry"/> values
		/// during updates.
		/// </summary>
...
//		}
		...
		public void BeginUpdate(IArchiveStorage archiveStorage, IDynamicDataSource dataSource)
		{
			...
			foreach(MyZipEntry entry in entries_) {
		...
		public void Add(string fileName, CompressionMethod compressionMethod, bool useUnicodeText )
		{
			...
			if (!MyZipEntry.IsCompressionMethodSupported(compressionMethod)) {
			...
			MyZipEntry entry = EntryFactory.MakeFileEntry(fileName);
		...
		/// </summary>
		/// <param name="fileName">The name of the file to add.</param>
		/// <param name="entryName">The name to use for the <see cref="MyZipEntry"/> on the Zip file created.</param>
		/// <exception cref="ArgumentNullException">Argument supplied is null.</exception>
		...
		public void Add(IStaticDataSource dataSource, string entryName, CompressionMethod compressionMethod)
		{
			...
			MyZipEntry entry = EntryFactory.MakeFileEntry(entryName, false);
			entry.CompressionMethod = compressionMethod;
		...
		public void AddDirectory(string directoryName)
		{
			...
			MyZipEntry dirEntry = EntryFactory.MakeDirectoryEntry(directoryName);
...
*/
		...
		/// <summary>
		/// Delete a <see cref="MyZipEntry"/> from the archive.
		/// </summary>
		/// <param name="entry">The entry to delete.</param>
		public void Delete(MyZipEntry entry)
		{
			if ( entry == null ) {
				throw new ArgumentNullException("entry");
			}
		...
		{
			MyZipEntry entry = update.OutEntry;
		...
		int WriteCentralDirectoryHeader(MyZipEntry entry)
		{
		...
		int FindExistingUpdate(MyZipEntry entry)
		{
			int result = -1;
 ...
 */
		...
		/// <summary>
		/// Get an output stream for the specified <see cref="MyZipEntry"/>
		/// </summary>
		/// <param name="entry">The entry to get an output stream for.</param>
		/// <returns>The output stream obtained for the entry.</returns>
		Stream GetOutputStream(MyZipEntry entry)
		{
		...
		{
			#region Constructors
			public ZipUpdate(string fileName, MyZipEntry entry)
			{
			...
			{
				command_ = UpdateCommand.Add;
				entry_ = new MyZipEntry(entryName);
			...
			public ZipUpdate(IStaticDataSource dataSource, MyZipEntry entry)
			{
				command_ = UpdateCommand.Add;
				entry_ = entry;
				dataSource_ = dataSource;
			...
			public ZipUpdate(MyZipEntry original, MyZipEntry updated)
			{
			...
			*/
			...
			public ZipUpdate(UpdateCommand command, MyZipEntry entry)
			{
				command_ = command;
				entry_ = ( MyZipEntry )entry.Clone();
			}
			...
			public ZipUpdate(MyZipEntry entry)
			...
			/// <summary>
			/// Get the <see cref="MyZipEntry"/> for this update.
			/// </summary>
			...
			/// <summary>
			/// Get the <see cref="MyZipEntry"/> that will be written to the updated/new file.
			/// </summary>
			public MyZipEntry OutEntry
			{
				get {
					if ( outEntry_ == null ) {
						outEntry_ = (MyZipEntry)entry_.Clone();
					}
		...
		void ReadEntries()
		{
			...
			entries_ = new MyZipEntry[entriesForThisDisk];
			...
			for (ulong i = 0; i < entriesForThisDisk; i++) {
				...
				MyZipEntry entry = new MyZipEntry(name, versionToExtract, versionMadeBy, (CompressionMethod)method);
		...
		/// </exception>
		long LocateEntry(MyZipEntry entry)
		{
			return TestLocalHeader(entry, HeaderTest.Extract);
		}
		
#if !NETCF_1_0		
		Stream CreateAndInitDecryptionStream(Stream baseStream, MyZipEntry entry)
		{
		...
		Stream CreateAndInitEncryptionStream(Stream baseStream, MyZipEntry entry)
		{
			CryptoStream result = null;
			if ( (entry.Version < ZipConstants.VersionStrongEncryption)
				|| (entry.Flags & (int)GeneralBitFlags.StrongEncryption) == 0) {
		...
		static void CheckClassicPassword(CryptoStream classicCryptoStream, MyZipEntry entry)
		{
		...
		/// <summary>
		/// An <see cref="IEnumerator">enumerator</see> for <see cref="MyZipEntry">Zip entries</see>
		/// </summary>
		class ZipEntryEnumerator : IEnumerator
		{
			#region Constructors
			public ZipEntryEnumerator(MyZipEntry[] entries)
			{
				array = entries;
			}
	...
	public interface IDynamicDataSource
	{
		...
		/// Get a data source.
		/// </summary>
		/// <param name="entry">The <see cref="MyZipEntry"/> to get a source for.</param>
		/// <param name="name">The name for data if known.</param>
		/// <returns>Returns a <see cref="Stream"/> to use for compression input.</returns>
		/// <remarks>Ideally a new stream is created and opened to achieve this, to avoid locking problems.</remarks>
		Stream GetSource(MyZipEntry entry, string name);
	}
	...
	public class DynamicDiskDataSource : IDynamicDataSource
	{
		...
		public Stream GetSource(MyZipEntry entry, string name)
		{
			Stream result = null;
